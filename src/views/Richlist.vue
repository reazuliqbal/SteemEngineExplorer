    ...
    <template v-if="dataLoaded">
      ...
      >
        ...
        <template v-slot:cell(account)="data">
          <a :href="`https://steemit.com/@${data.item.account}`">{{data.item.account}}</a>
        </template>
...
export default {
  ...
  async created() {
    ...
    if (!symbol) {
      ...
      this.symbol = 'ENG';
      this.$router.push({ name: 'richlist', params: { symbol: 'ENG' } });
    } else {
      this.symbol = symbol;
    }
  ...
  methods: {
    async loadRichlist(token) {
      ...
      let offset = 0;
      const query = await SE.getTokenHolders(token, offset);

      offset += 1000;
      let newHolders = await SE.getTokenHolders(token, offset);

      while (newHolders.length > 0) {
        query.push(...newHolders);
        offset += 1000;
        // eslint-disable-next-line no-await-in-loop
        newHolders = await SE.getTokenHolders(token, offset);
      }
